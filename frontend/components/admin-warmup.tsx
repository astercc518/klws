...
const PAGE_SIZE = 20;
const LANES: Lane[] = ["FAST", "STANDARD"];
...
// PoliciesDialog reads/writes the two lane policies (FAST/STANDARD) in one
// shot: GET returns both keyed by lane, PUT saves one lane at a time (per the
// API's own :lane path param), so Save fires up to two sequential requests.
function PoliciesDialog({ open, onOpenChange }: { open: boolean; onOpenChange: (o: boolean) => void }) {
  const t = useT();
